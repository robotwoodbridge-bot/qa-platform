...
## TypeScript (repo-wide, TS 7.0.2 / tsgo)

- **`.vscode/settings.json`** (root) sets `js/ts.experimental.useTsgo: true`
  and is deliberately tracked — `.gitignore` has `!/.vscode/` specifically
  because this is the *only* place VS Code reads `.vscode/settings.json`
  from when the repo root is opened as the workspace; a nested
  `<project>/.vscode/` would silently never apply. Don't add per-project
  `.vscode/` folders expecting them to contribute settings when the repo is
  opened at the root — they won't.
- **`tsconfig.base.json`** (root) holds shared compiler options (target,
  strict mode, module resolution, etc.) for every TypeScript project in
  this repo. It has no `include`/`rootDir`/`outDir` of its own — it's not
  meant to build anything by itself, only to be extended.
- Each actual TS project gets its own `tsconfig.json` that
  `"extends": "<relative-path-to-repo-root>/tsconfig.base.json"` and adds
  only its own `rootDir`/`outDir`/`include`/`exclude`. `temp/TypeScript/tsconfig.json`
  is the current example — follow that shape for `testings/gui/playwright/`
  (once that suite exists) and `platform/reporting/schema/` (once its
  `.ts` files exist) rather than duplicating the shared options per project.
