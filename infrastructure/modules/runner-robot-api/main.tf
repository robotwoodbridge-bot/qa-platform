# Robot Framework + RequestsLibrary — HTTP-only API suites. No browser, no
# Docker socket; just Python hitting the API under test directly.
resource "null_resource" "build" {
  triggers = {
    dockerfile   = filemd5(var.dockerfile_path)
    requirements = filemd5("${var.build_context_path}/infrastructure/docker/requirements-robot-api.txt")
